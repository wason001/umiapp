          ...
          <div className="w-full flex justify-center my-24">
            <div className="w-full container lg:px-64 px-8">
              {post.content.split('\n').map((line: string, i: number) => {
                if (line.startsWith('# '))
                  return (
                    <p key={i} className="text-3xl font-extrabold mt-24 mb-12">
                      {line.split('# ')[1]}
                    </p>
                  );
                if (line.startsWith('## '))
                  return (
                    <p key={i} className="text-2xl font-extrabold mt-24 mb-12">
                      {line.split('## ')[1]}
                    </p>
                  );
                if (line.startsWith('### '))
                  return (
                    <p key={i} className="text-xl font-extrabold mt-24 mb-12">
                      {line.split('### ')[1]}
                    </p>
                  );
                if (line.startsWith('![]('))
                ...
                return (
                  <p key={i} className="text-zinc-600 mt-2">
                    {line}
                  </p>
                );
              })}
